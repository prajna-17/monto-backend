...
export const createOrder = async (req, res) => {
	try {
		...
		res.status(201).json(
			createResponse(201, savedOrder, "order place successfully")
		);
	} catch (error) {
...
export const fetchUserAllOrders = async (req, res) => {
	try {
		...
		if (!userDetails) {
			return res
				.status(400)
				.json(ErrorResponse(400, "you can access orders section"));
		}
...
export const fetchOrderDetails = async (req, res) => {
	try {
		...
		if (!orderDetails) {
			return res
				.status(400)
				.json(ErrorResponse(400, "order are not valid"));
		}

		return res
			.status(200)
			.json(
				createResponse(200, orderDetails, "order details fetched")
			);
	} catch (error) {
		res.status(500).json(ErrorResponse(500, error.message));
	}
